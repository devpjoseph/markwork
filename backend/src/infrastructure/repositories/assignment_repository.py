...
class AssignmentRepository(IAssignmentRepository):
    ...
    async def create(
        ...
        assignment = Assignment(
            student_id=student_id, teacher_id=teacher_id, title=title
        )
        self._session.add(assignment)
        await self._session.flush()

        version = AssignmentVersion(
            assignment_id=assignment.id, version_number=1, content=initial_content
        )
    ...
    async def add_version(
        ...
        count_result = await self._session.execute(
            select(func.count())
            .select_from(AssignmentVersion)
            .where(AssignmentVersion.assignment_id == assignment_id)
        )
        next_number = (count_result.scalar() or 0) + 1

        version = AssignmentVersion(
            assignment_id=assignment_id, version_number=next_number, content=content
        )
        self._session.add(version)
        await self._session.flush()
    ...
    async def get_versions(
        ...
        return [
            AssignmentVersionEntity.model_validate(v) for v in result.scalars().all()
        ]

    async def _fetch_latest_version_orm(self, assignment_id: uuid.UUID):
        """Shared query for the latest version ORM object."""
        result = await self._session.execute(
            select(AssignmentVersion)
            .where(AssignmentVersion.assignment_id == assignment_id)
    ...
    async def update_latest_version(
        ...
        version = await self._fetch_latest_version_orm(assignment_id)
        if version is None:
            raise ValueError(f"No version found for assignment {assignment_id}")
